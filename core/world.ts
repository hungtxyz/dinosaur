...
class WorldObject {
    ...
    LoadModel_() {
        ...
        loader.load('Cactus3.fbx', (fbx) => {
            ...
            fbx.traverse(c => {
                if ((<any>c).geometry) {
                    (<any>c).geometry.computeBoundingBox();
                }
...
export class WorldManager {
    ...
    private scoreText_: string;
    private separationDistance_: number;
    constructor(params: any) {
        this.objects_ = [];
        this.unused_ = [];
        this.speed_ = 12;
    ...
    SpawnObj_(scale: number, offset: number) {
        let obj = null;
    ...
    SpawnCluster_() {
        const scaleIndex = MathUtils.randInt(0, 1);
        const scales = [1, 0.5];
        const ranges = [2, 3];
        const scale = scales[scaleIndex];
        const numObjects = MathUtils.randInt(1, ranges[scaleIndex]);
    ...
    MaybeSpawn_() {
        ...
        if (Math.abs(START_POS - closest) > this.separationDistance_) {
            this.SpawnCluster_();
            this.separationDistance_ = MathUtils.randRange(SEPARATION_DISTANCE, SEPARATION_DISTANCE * 1.5);
        }
    }
    ...
    UpdateScore_(timeElapsed:number) {
    ...
    UpdateColliders_(timeElapsed:number) {
        const invisible = [];
        const visible = [];
